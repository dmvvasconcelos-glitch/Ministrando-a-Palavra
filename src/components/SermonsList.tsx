...
// Optimized Sermon Card as a standalone memoized component
const SermonCard = memo(({ 
  sermon, 
  deletingId, 
  setDeletingId, 
  handleDelete, 
  ...
  onPreach: (id: string) => void,
  t: (key: any) => string,
  formatSermonDate: (date: any) => string
}) => {
...
export default function SermonsList({ onEdit, onPreach, onNew }: SermonsListProps) {
  ...
  const formatSermonDateInList = useMemo(() => {
    return (date: any) => {
      if (!date) return '';
  ...
  return (
    <div className="space-y-6">
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
        <div>
          <h1 className="text-xl font-bold tracking-tight text-app-text flex items-center gap-2">
            <div className="w-1.5 h-5 bg-app-accent rounded-full opacity-60" />
            {t('myMinistries')}
          </h1>
          <p className="text-xs text-app-secondary font-medium tracking-wide opacity-70 mt-0.5">{t('myMinistriesSub')}</p>
        </div>
        <button 
          onClick={onNew}
        ...
        <AnimatePresence mode="popLayout">
          {filteredSermons.map((sermon) => (
            <SermonCard
              ...
              onPreach={onPreach}
              t={t}
              formatSermonDate={formatSermonDateInList}
            />
          ))}
        </AnimatePresence>
